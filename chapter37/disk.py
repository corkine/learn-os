...
class Disk:
    def __init__(self, addr, addrDesc, lateAddr, lateAddrDesc,
        ...
        if self.graphics:
            self.root = Tk()
            self.root.title("Disk Simulator")
        ...
        # TIME INFO
        if self.graphics:
            self.timeID = self.canvas.create_text(10, 10, text='时间: 0.00', anchor='w')
            self.canvas.create_rectangle(95,0,200,18, fill='orange', outline='orange')
            self.seekID = self.canvas.create_text(100, 10, text='寻道: 0.00', anchor='w')
            self.canvas.create_rectangle(195,0,300,18, fill='lightblue', outline='lightblue')
            self.rotID  = self.canvas.create_text(200, 10, text='旋转: 0.00', anchor='w')
            self.canvas.create_rectangle(295,0,400,18, fill='green', outline='green')
            self.xferID = self.canvas.create_text(300, 10, text='传输: 0.00', anchor='w')
            self.canvas.create_text(320, 40, text='按下 "s" 开始', anchor='w')
            self.canvas.create_text(320, 60, text='按下 "p" 暂停', anchor='w')
            self.canvas.create_text(320, 80, text='按下 "q" 重启', anchor='w')
        self.timer = 0
    ...
    def UpdateTime(self):
        if self.graphics:
            self.canvas.itemconfig(self.timeID, text='时间: ' + str(self.timer))
            self.canvas.itemconfig(self.seekID, text='寻道: ' + str(self.seekTotal))
            self.canvas.itemconfig(self.rotID,  text='旋转: ' + str(self.rotTotal))
            self.canvas.itemconfig(self.xferID, text='传输: ' + str(self.xferTotal))
